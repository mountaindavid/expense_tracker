...
class TestExpenseServiceCreate:
    ...
    def test_create_success(self, mock_conn):
        ...
        with patch('app.services.expenses.create_expense', return_value=expected_response) as mock_create:
...
class TestExpenseServiceGetAll:
    ...
    def test_get_all_success(self, mock_conn):
        ...
        with patch('app.services.expenses.get_all_expenses', return_value=expected_responses) as mock_get_all:
            service = ExpenseService(mock_conn)
    ...
    def test_get_all_database_error(self, mock_conn):
        """Test that psycopg2.Error is converted to DatabaseError"""
        with patch('app.services.expenses.get_all_expenses', side_effect=psycopg2.Error("DB error")):
            service = ExpenseService(mock_conn)
            with pytest.raises(DatabaseError) as exc_info:
                service.get_all()
            assert "Database error: DB error" in str(exc_info.value)
...
class TestExpenseServiceGetById:
    ...
    def test_get_by_id_success(self, mock_conn):
        ...
        with patch('app.services.expenses.get_expense_by_id', return_value=expected_response) as mock_get_by_id:
            service = ExpenseService(mock_conn)
    ...
    def test_get_by_id_database_error(self, mock_conn):
        """Test that psycopg2.Error is converted to DatabaseError"""
        with patch('app.services.expenses.get_expense_by_id', side_effect=psycopg2.Error("DB error")):
            service = ExpenseService(mock_conn)
            with pytest.raises(DatabaseError) as exc_info:
                service.get_by_id(1)
            assert "Database error: DB error" in str(exc_info.value)
...
class TestExpenseServiceUpdate:
    ...
    def test_update_success(self, mock_conn):
        ...
        with patch('app.services.expenses.update_expense', return_value=expected_response) as mock_update:
            service = ExpenseService(mock_conn)
